          ...
          setTimeout(async () => {
            try {
              ...
              if (session?.user) {
                ...
                console.log('登录后检查管理员权限:', { userId: session.user.id, isAdmin: adminStatus });
                
                if (adminStatus) {
                  // 是管理员，直接跳转到后台（使用 window.location 确保跳转）
                  console.log('检测到管理员，跳转到后台');
                  window.location.href = '/admin';
                  return; // 不调用 onClose，避免被覆盖
                }
              }
            } catch (err) {
